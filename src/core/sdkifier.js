...
module.exports = {
  generatePlatformSdk: async (baseUrl, authHeader, label, dir) => {
    ...
    return swaggerer(label || 'platformSDK', swagger.body, dir);
  },
  generateHubSdk: async (hub, baseUrl, authHeader, label, dir) => {
    ...
    if (hubs.includes(hub)) {
      ...
      return swaggerer(label || `${hub}SDK`, swagger.body, dir);
    }
    else {
  ...
  generateElementSdk: async (key, id, baseUrl, authHeader, label, dir) => {
    ...
    try {
      ...
      return swaggerer(label || `${keyOrId}SDK`, swagger.body, dir);
    } catch (e) {
      if (e.status === 404) {
        if (authHeader) {
  ...
  generateInstanceSdk: async (id, baseUrl, authHeader, label, dir) => {
    ...
    try {
      ...
      return swaggerer(label || `_${id}SDK`, swagger.body, dir);
    } catch (e) {
      if (e.status === 404) {
